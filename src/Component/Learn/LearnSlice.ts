...
const fetchCardsForLearn = createAppAsyncThunk<ResponseGetCardsType, { cardsPack_id: string }>(
  'cards/setCards',
  async (arg, thunkAPI) => {
    ...
    try {
      ...
      const cards = await cardsApi.getCards(arg.cardsPack_id)
...
const updateCardForLearn = createAppAsyncThunk<
  CardType,
  { _id: string | undefined; grade: number }
>('cards/updateCard', async (arg, thunkAPI) => {
  const { dispatch, rejectWithValue } = thunkAPI
